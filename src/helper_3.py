...
def Extract_variable_points(signal, time, proposition, alph1, alph2, pos):  #"s==4.2", "ss<10"
    # add signal at time=0
    change_points = [(time[0], signal[0])]
    # Get the ordered list of time keys
    time_keys = list(signal.keys())
    ...
    for i in range(1, len(time_keys)):
        ...
        prop_current = proposition.replace("s", f"signal[{current_time}]")
        prop_previous = proposition.replace("s", f"signal[{previous_time}]")
    ...
    number = re.search(r'\b\d+\.\d+\b', proposition)
    ...
    "<": operator.lt,
    ">": operator.gt}
    operatorfound = re.search(r'(==|<=|>=|<|>)', proposition)
    ...
    for i, value in enumerate(change_points):
        if comparison_function and comparison_function(value[1], number):
            ...
            inputTimedWord[i][pos] = alph1    
        else:
            inputTimedWord.append([value, '','','','']) # not_a
            inputTimedWord[i][pos] = alph2
    return  inputTimedWord
